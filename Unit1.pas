...
if map=0 then StatusBar1.SimpleText:='Ceйчас редактируется нулевая поверхность' ;
if map=1 then StatusBar1.SimpleText:='Ceйчас редактируется первая поверхность' ;
if map=2 then StatusBar1.SimpleText:='Ceйчас редактируется вторая поверхность' ;
...
rewrite(fm);
    write(fm,'Основной файл для сборки карты.');
    write(fm,'Файлы-компоненты:');
